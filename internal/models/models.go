...
// Widget is the type of all widgets
type Widget struct {
	ID             int       `json:"id"`
...
// Status is the type for order sttauses
type Status struct {
	ID        int       `json:"id"`
	Name      string    `json:"name"`
	CreatedAt time.Time `json:"-"`
...
func (m *DBModel) InsertTransaction(txn Transaction) (int, error) {
	...
	query :=
		...
		insert into transactions
			(amount,currency, last_four, bank_return_code, expiry_month, expiry_year,payment_intent,
...
func (m *DBModel) InsertOrder(order Order) (int, error) {
	...
	query :=
		...
		values(?,?,?,?,?,?,?,?)	
	`
	result, err := m.DB.ExecContext(ctx, query, order.WidgetID, order.TransactionID, order.StatusID,
		order.Quantity, order.CustomerID, order.Amount, time.Now(), time.Now())
	if err != nil {
		return 0, err
	}
	id, err := result.LastInsertId()
